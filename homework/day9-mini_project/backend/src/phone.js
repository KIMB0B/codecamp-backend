...
export async function isAuthPhone(phone) {
    const findResult = await TokenCollection.findOne({
        phone: phone,
    });
    try {
...
export async function setTokenInDB(phone, token) {
    try {
...
export async function checkToken(phone, token) {
    const accordPhone = await TokenCollection.findOne({ phone: phone }).exec();
    if (!accordPhone) {
        console.log(
